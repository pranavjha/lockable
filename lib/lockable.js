...
module.exports = {
    ...
    // Sequence is used to _sequencify_ the resolution of promises. Promises added in a sequence will be resolved in the
    ...
    sequence: function(sequenceId, startCb, endCb) {
        ...
        return function(promise) {
            ...
            return Promise.all(promiseSequence).then(function(thisValue) {
    ...
    // A queue delays the execution of functions till the return value of previous execution is resolved. This is
    // different from a sequence where, the actual promises are invoked immediately and only the resolution is delayed.
    //
    // A queue can be used when multiple executions have to happen only one after the other (eg, updating the same
    // records or autosave in a UI form)
    //
